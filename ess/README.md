...
| `doc_list.py` | (신규) | `regulations_list.json` / `project_list.json` / `test_cases_list.json` 문서 레지스트리 관리 |
...
| Test case 저장 | `.session_storage/{user}/ess/test_cases/` |
| Test case 목록 | `.session_storage/{user}/ess/test_cases_list.json` |
| 변환 결과 | 원본과 동일 폴더의 `{stem}.md` + `{stem}.json` |
| FMP 페이지/중간 | `.../out/converted/.pdf_pages/{stem}_{hash}/pages/` + `extracted.md` |

`regulations_list.json` / `project_list.json` / `test_cases_list.json`은 `ess/doc_list.py`가 관리하며, 문서 추가(업로드)와 Sync 추출 완료(또는 테스트케이스 「저장」) 시마다 갱신됩니다.
